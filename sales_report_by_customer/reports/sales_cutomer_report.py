...
class SalesByCustomer(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        for partner in list(set(customers.mapped('partner_id.id'))) \
                if data['form']['report_for'] == 'customer' \
                else list(set(customers.mapped('invoice_user_id.id'))):
            domain = [('invoice_date', '>=', date_from),
                      ('invoice_date', '<=', date_to), ('state', '=', 'posted')]
            if data['form']['inv_cr'] == 'invoice':
                domain += [('move_type', '=', 'out_invoice')]
            if data['form']['inv_cr'] == 'credit_note':
                domain += [('move_type', '=', 'out_refund')]
            if data['form']['report_for'] == 'customer':
                domain += [('partner_id', '=', partner)]
                invoice_search = self.env['account.move'].search(domain)
            else:
                domain += [('invoice_user_id', '=', partner)]
                invoice_search = self.env['account.move'].search(domain)
